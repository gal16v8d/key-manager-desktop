...
public class UsuarioEjb implements Ejb<Usuario> {
    ...
    public List<Usuario> list() {
        ...
        try {
            if (Long.valueOf(RolEnum.ADMIN.getCode()).equals(SessionData.getInstance().getSessionDto().getRol())) {
                DBConnection.getInstance()
                        .setPst(DBConnection.getInstance().getCon().prepareStatement(QueryConstants.USER_LIST_ADMIN));
            } else {
                DBConnection.getInstance()
